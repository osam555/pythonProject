...
def audio_autoplay(file_name, play_speed, repeat_num):
    ...
    with mymidia_placeholder:
        ...
        time.sleep(0.5 + (get_audio_length(file_name) / play_speed) * repeat_num)
...
def setting_lang(lang):
    ...
    with st.expander(f"{lang} 설정"):
        interval_time = st.slider("간격 (초)", 0.0, 5.0, step=0.1, key=lang,
                                  value=float(get_saved_data["간격"]) if get_saved_data else 0.1)
        ...
        if sound:
            sound_speed = st.slider("배속", 1.0, 6.0, step=0.1, key=lang,
                                    value=float(get_saved_data["배속"]) if get_saved_data else 0.0)
            sound_repeat_num = st.slider("반복 횟수", 1, 5, step=1, key=lang,
                                         value=int(get_saved_data["반복횟수"]) if get_saved_data else 0)
...
if ip_address in list(f_setting_dict.keys()):
    ...
    setting_dict = {
        ...
        "끝번호": 600,
        "자막크기": 1,
        "언어설정": [{"언어": "한국어", "자막": True, "간격": 0.0, "소리": True, "배속": 1.0, "반복횟수": 1},
    ...
    for i in df.itertuples():
        ...
        for j in i[1:]:
            ...
            if setting_list[num - 1]["소리"] == True:
                audio_autoplay(
                    f"./음성파일/{selected_file_name.replace('.xlsx', '')}/{setting_list[num - 1]['언어']}/{i[0]}.mp3",
                    play_speed=setting_list[num - 1]["배속"], repeat_num=setting_list[num - 1]["반복횟수"])
